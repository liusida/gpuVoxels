...
public slots: //global slot repository for updating things across all windows/views...
    ...
    void ImportHistory() {ForceViewMode();QString FileName; if (!MainSim.OpenHistory(&FileName)) return; FastModeChanged(false); CheckNumVox(); ZoomExtAll(); SetWindowName(FileName); UpdateAllWins(); PhysicsMode(true); MainSim.pSimView->SetCurViewVox(RVV_HISTORY); MainSim.LockCoMToCenter=true; }
