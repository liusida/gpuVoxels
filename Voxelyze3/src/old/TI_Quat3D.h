#if !defined(TI_TI_QUAT3D_H)
#define TI_TI_QUAT3D_H
...
//		if (theta < DISCARD_ANGLE_RAD) {*this = TI_Quat3D(1,0,0,0); return;} //very small angle, return no rotation
		...
		TI_Vec3D<T> Axis = RotateFrom.Cross(RotateTo); //Axis of rotation
		Axis.NormalizeFast();
		if (theta > PI-DISCARD_ANGLE_RAD) {*this = TI_Quat3D(Axis); return;} //180 degree rotation (180 degree rot about axis ax, ay, az is Quat(0,ax,ay,az))
	...
	CUDA_DEVICE void FromAngleToPosX(const TI_Vec3D<T>& RotateFrom){ //highly optimized at the expense of readability
		...
		TI_Vec3D<> RotFromNorm = RotateFrom;
		...
		//Here theta is the angle, axis is RotFromNorm.Cross(TI_Vec3D(1,0,0)) = TI_Vec3D(0, RotFromNorm.z/AxisMagInv, -RotFromNorm.y/AxisMagInv), which is still normalized. (super rolled together)
		const T a = 0.5*theta;
		const T s = sin(a);
	...
	template <typename U> CUDA_DEVICE const TI_Vec3D<U> RotateVec3DInv(const TI_Vec3D<U>& f) const {
		...
		T ty = w*fy + x*fz - z*fx;
		T tz = w*fz - x*fy + y*fx;
		return TI_Vec3D<U>(tw*x + tx*w + ty*z - tz*y, tw*y - tx*z + ty*w + tz*x, tw*z + tx*y - ty*x + tz*w);	
