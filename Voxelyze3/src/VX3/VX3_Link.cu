...
__device__ VX3_Quat3D<double> VX3_Link::orientLink(/*double restLength*/) //updates pos2, angle1, angle2, and smallAngle
{
	pos2 = toAxisX(VX3_Vec3D<double>(pVPos->position() - pVNeg->position())); //digit truncation happens here...

	angle1 = toAxisX(pVNeg->orientation());
	angle2 = toAxisX(pVPos->orientation());

	VX3_Quat3D<double> totalRot = angle1.Conjugate(); //keep track of the total rotation of this bond (after toAxisX())
...
__device__ void VX3_Link::updateForces()
{
	...
	VX3_Vec3D<double> oldAngle2v = angle2v; //remember the positions/angles from last timestep to calculate velocity

	orientLink(/*restLength*/); //sets pos2, angle1, angle2
	//time 87.876us
	VX3_Vec3D<double> dPos2 = 0.5*(pos2-oldPos2); //deltas for local damping. velocity at center is half the total velocity
	...
	//time 119.13us
	if (isFailed()){forceNeg = forcePos = momentNeg = momentPos = VX3_Vec3D<double>(0,0,0); return;}
	//time 120.48us
	...
	momentNeg = VX3_Vec3D<double> (	a2*(angle2v.x - angle1v.x),
								-b2*pos2.z - b3*(2*angle1v.y + angle2v.y),
								b2*pos2.y - b3*(2*angle1v.z + angle2v.z));
	momentPos = VX3_Vec3D<double> (	a2*(angle1v.x - angle2v.x),
	...
	if (isLocalVelocityValid()){ //if we don't have the basis for a good damping calculation, don't do any damping.
		...
		VX3_Vec3D<double> posCalc(	sqA1*dPos2.x,
		...
		momentNeg -= 0.5*pVNeg->dampingMultiplier()*VX3_Vec3D<>(	-sqA2xIp*(dAngle2.x - dAngle1.x),
																sqB2xFMp*dPos2.z + sqB3xIp*(2*dAngle1.y + dAngle2.y),
																-sqB2xFMp*dPos2.y + sqB3xIp*(2*dAngle1.z + dAngle2.z));		
		momentPos -= 0.5*pVPos->dampingMultiplier()*VX3_Vec3D<>(	sqA2xIp*(dAngle2.x - dAngle1.x),
