...
VX3_Link::VX3_Link(CVX_Link *p, VX3_VoxelyzeKernel *k)
    : forceNeg(p->forceNeg), forcePos(p->forcePos), momentNeg(p->momentNeg),
      momentPos(p->momentPos), strain(p->strain), maxStrain(p->maxStrain),
      strainOffset(p->strainOffset), boolStates(p->boolStates), axis(p->axis),
      strainRatio(p->strainRatio), pos2(p->pos2), angle1v(p->angle1v),
      angle2v(p->angle2v), angle1(p->angle1), angle2(p->angle2),
      smallAngle(p->smallAngle), currentRestLength(p->currentRestLength),
      currentTransverseArea(p->currentTransverseArea),
...
__device__ VX3_Link::VX3_Link(VX3_Voxel *voxel1, linkDirection dir1,
...
__device__ float VX3_Link::updateStrain(float axialStrain) {
    ...
    if (mat->linear) {
        ...
        if (axialStrain >
            maxStrain) { // if new territory on the stress/strain curve
            maxStrain = axialStrain; // remember this maximum for easy reference
            ...
            if (mat->nu != 0.0f)
                strainOffset =
                    maxStrain -
                    mat->stress(axialStrain) /
                        (mat->_eHat *
                         (1 - mat->nu)); // precalculate strain offset for when
                                         // we back off
            else
                strainOffset =
                    maxStrain -
                    returnStress / mat->E; // precalculate strain offset for
                                           // when we back off

        } else { // backed off a non-linear material, therefore in linear
                 // region.

            float relativeStrain =
                axialStrain - strainOffset; // treat the material as linear with
                                            // a strain offset according to the
                                            // maximum plastic deformation

            if (mat->nu != 0.0f)
                returnStress = mat->stress(relativeStrain,
                                           currentTransverseStrainSum, true);
            else
...
__device__ float VX3_Link::strainEnergy() const {
    return forceNeg.x * forceNeg.x / (2.0f * mat->_a1) +  // Tensile strain
           ...
           (momentNeg.z * momentNeg.z - momentNeg.z * momentPos.z +
            momentPos.z * momentPos.z) /
               (3.0 * mat->_b3) + // Bending Z
           (momentNeg.y * momentNeg.y - momentNeg.y * momentPos.y +
            momentPos.y * momentPos.y) /
               (3.0 * mat->_b3); // Bending Y
}
...
__device__ float VX3_Link::axialStiffness() {
    if (mat->isXyzIndependent())
        ...
        return (float)(mat->_eHat * currentTransverseArea /
                       ((strain + 1) * currentRestLength)); // _a1;
    }
}
