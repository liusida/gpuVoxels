...
__device__ void VX3_Voxel::timeStep(double dt, double currentTime, VX3_VoxelyzeKernel* k)
{
	...
	// Apply Force Field
	curForce.x += k->force_field.x_forcefield(pos.x, pos.y, pos.z, currentTime);
	curForce.y += k->force_field.y_forcefield(pos.x, pos.y, pos.z, currentTime);
	curForce.z += k->force_field.z_forcefield(pos.x, pos.y, pos.z, currentTime);
